 ...
 */
bool cellift_logic_or(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    if (data_size == 1) // Equivalent to traditional or operator.
        for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
            ...
            RTLIL::SigSpec a_taint_and_not_b = module->And(NEW_ID, port_taints[A][taint_id], not_b);
            RTLIL::SigSpec b_taint_and_not_a = module->And(NEW_ID, port_taints[B][taint_id], not_a);
        ...
        for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
            ...
            // Instantiate the tainted assignments for A and B that maximize/minimize the possibility of a $logic_or.
            ...
            RTLIL::SigSpec min_or = module->Or(NEW_ID, min_a_bit, min_b_bit);
            RTLIL::SigSpec max_or = module->Or(NEW_ID, max_a_bit, max_b_bit);

            // Xor them to see if the output can be influenced by tainted inputs.
            module->addXor(NEW_ID, min_or, max_or, port_taints[Y][taint_id][0]);
