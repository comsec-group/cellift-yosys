...
struct InsertBmuxCellWorker {
private:
	RTLIL::Module *module = nullptr;
	bool is_multiple = false;
	const RTLIL::IdString insert_bmux_cell_attribute_name = ID(insert_bmux_cell);

	void insert_bmux_cell() {

		if (module->get_bool_attribute(insert_bmux_cell_attribute_name))
			return;

		log("Inserting bmux cell...\n");
		...
		do {
			...
			new_cell = module->addBmux(NEW_ID, wire_a_i, wire_s_i, wire_y_o);
			...
			module->set_bool_attribute(insert_bmux_cell_attribute_name, true);
			multiple_id++;
		} while (is_multiple);
	}

public:
	InsertBmuxCellWorker(RTLIL::Module *_module, bool _is_multiple) {
		module = _module;
...
struct InsertBmuxCellPass : public Pass {
	InsertBmuxCellPass() : Pass("insert_bmux_cell", "Add a bmux cell in the design top module. This is a pass dedicated to cell unit testing.") {}

	void help() override
	{
		//   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
		log("\n");
		log("    insert_bmux_cell --multiple\n");
		log("\n");
		log("Add a bmux cell in each module of the design.\n");
		log("This pass has been designed for testing the bmux implementation.\n");
	...
	{
		log_header(design, "Executing insert_bmux_cell pass.\n");
		...
		InsertBmuxCellWorker worker(design->top_module(), is_multiple);
	}
} InsertBmuxCellPass;
