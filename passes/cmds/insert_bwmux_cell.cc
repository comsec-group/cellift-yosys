...
struct InsertBwmuxCellWorker {
private:
	RTLIL::Module *module = nullptr;
	bool is_multiple = false;
	const RTLIL::IdString insert_bwmux_cell_attribute_name = ID(insert_bwmux_cell);

	void insert_bwmux_cell() {

		if (module->get_bool_attribute(insert_bwmux_cell_attribute_name))
			return;

		log("Inserting bwmux cell...\n");
		...
		do {
			...
			new_cell = module->addBwmux(NEW_ID, wire_a_i, wire_b_i, wire_s_i, wire_y_o);
			...
			module->set_bool_attribute(insert_bwmux_cell_attribute_name, true);
			multiple_id++;
		} while (is_multiple);
	}

public:
	InsertBwmuxCellWorker(RTLIL::Module *_module, bool _is_multiple) {
		module = _module;
...
struct InsertBwmuxCellPass : public Pass {
	InsertBwmuxCellPass() : Pass("insert_bwmux_cell", "Add a bwmux cell in the design top module. This is a pass dedicated to cell unit testing.") {}

	void help() override
	{
		//   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
		log("\n");
		log("    insert_bwmux_cell --multiple\n");
		log("\n");
		log("Add a bwmux cell in each module of the design.\n");
		log("This pass has been designed for testing the bwmux implementation.\n");
	...
	{
		log_header(design, "Executing insert_bwmux_cell pass.\n");
		...
		InsertBwmuxCellWorker worker(design->top_module(), is_multiple);
	}
} InsertBwmuxCellPass;
